...
public class Shockwave {
    public double x, y;      // origin (px)
    public double radius = 0.0;
    public final double maxRadius;
    public final double speed; // px / s
    public final double thickness; // stroke thickness
    ...
    // check whether a brick center is within current radius (we consider wave touches when radius >= dist)
    public boolean touchesBrick(Brick b) {
        ...
        // We want shock to "affect" brick when radius passes dist. Use thickness to allow window:
        return dist <= radius && dist >= (radius - thickness - 1.0);
    }

    public void render(GraphicsContext gc) {
        // ring fill (soft) + stroke
        gc.save();
        // soft purple fill ring using additive glow
